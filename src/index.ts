...
import {FeedDialog} from './dialogs/feedDialog';
const FEEDBACK_DIALOG = 'feedDialog';
...
//const mainDialog = new MainDialog (MAIN_DIALOG);
const bookDialog = new BookDialog(BOOKING_DIALOG);
const flightBookingRecognizer =new FlightBookingRecognizer(FLIGHT_BOOKING_RECOGNIZER);
//const feedBackDialog = new FeedBackDialog(FEEDBACK_DIALOG);
const dialog = new MainDialog(luisRecognizer, bookDialog, flightBookingRecognizer );
const bot = new DialogAndWelcomeBot(conversationState, userState, dialog);
// Create HTTP server
const server = restify.createServer();
server.listen(process.env.port || process.env.PORT || 8085, () => {
